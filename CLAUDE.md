...
## Per-run selection = cost-aware Thompson sampling, NOT rank-#1. Two levers, two decision points.

The campaign picks, per run, WHICH audience to contact and WHICH workflow to run. Both are chosen by a cost-aware Thompson bandit (`src/lib/bandit.ts`) — sample each arm's success rate from `Beta(successes+1, trials−successes+1)` (evidence from features-service, no local state), pick `argmin(costPerTrial / sampledRate)`. Mostly exploits the best arm, keeps exploring the rest so none stays permanently shadowed. Do NOT revert to "take features-service rank #1" — that froze every campaign on one audience+workflow.
...
**Follow-up not yet shipped:** features-service `/candidates` returns `audienceId: null` (audience grain inert) — until it's populated, the audience leg runs over `/audience-stats` (workflow-agnostic), so audience and workflow vary independently (factored `P(w)·P(a|w)`), not as a single joint cell. Conditioning the audience on the chosen workflow needs the features-service audience grain. (Set 2026-06-21, campaign-service#219.)
