...
function campaignForRow(
  ...
  const byFunnel = all.filter((c) => {
    ...
    if (row.funnelKey === null || !c.funnelKey) {
      ...
      // LEG-LESS money, claimed by a campaign stating no funnel only where `offerLegCeilingCents`
      // lets it spend it: no ceiling of the brand names a leg, and this offer's leg-less rows on
      // this channel sit under exactly ONE funnel.
      if (c.funnelKey || !c.legKey) return false;
