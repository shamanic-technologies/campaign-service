...
const body = { brandId: BRAND, offerId: OFFER, funnelKey: FUNNEL, featureSlug: CHANNEL };
...
describe("POST /campaigns/start-funded-pair", () => {
  beforeEach(async () => {
    ...
    mockResolve.mockResolvedValue({
      ok: true,
      pair: { funnelKey: FUNNEL, legKey: LEG, ceilingCents: 2500, workflowSlug: "aurora-v3" },
    });
  ...
  it("starts the campaign for a funded pair without a workflow, a name or a budget", async () => {
    ...
    expect(campaign.funnelKey).toBe(FUNNEL);
    ...
    expect(campaign.name).toContain(FUNNEL);
  ...
  it("adopts the campaign already doing this work rather than twinning it, when it states no leg", async () => {
    mockResolve.mockResolvedValueOnce({
      ok: true,
      pair: { funnelKey: FUNNEL, legKey: null, ceilingCents: 4000, workflowSlug: "aurora-v3" },
    });
    const legless = await post(body).expect(201);
    expect(legless.body.campaign.legKey).toBeNull();

    // The customer now funds the same pair at the leg grain. That is the SAME campaign, which
    // learns the leg it was bought for.
    const adopted = await post(body).expect(200);
    expect(adopted.body.campaign.id).toBe(legless.body.campaign.id);
    expect(adopted.body.campaign.legKey).toBe(LEG);
  ...
  it("renders a refusal a person can read, with a code a consumer can branch on", async () => {
    mockResolve.mockResolvedValue({
      ...
      refusal: {
        status: 409,
        code: "not_funded",
        message: "You haven't funded this channel for that sales funnel. Set its daily budget, then start it.",
      },
    });
  ...
  it("gives a CUSTOMER-operated channel a campaign with no workflow, and never triggers one", async () => {
    mockResolve.mockResolvedValue({
      ok: true,
      pair: { funnelKey: FUNNEL, legKey: LEG, ceilingCents: 2500, workflowSlug: null },
    });
